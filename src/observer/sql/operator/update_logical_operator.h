...
{
public:
  UpdateLogicalOperator(Table *table, const std::unordered_map<std::string, std::shared_ptr<Expression>> &kv_pairs)
...
private:
  ...
  const std::unordered_map<std::string, std::shared_ptr<Expression>> kv_pairs_;
