    ...
    if (nullptr == table) {
      ...
      if (nullptr == table){ 
        ...
        if(db->find_table_by_alias(alias.c_str()) != nullptr){
          if (db->find_table_by_alias(alias.c_str())->name() != table_name1 && select_sql.sub_select == nullptr){
...
RC SelectStmt::create(Db *db, SelectSqlNode &select_sql, Stmt *&stmt , int has_subselect)
{
  if (nullptr == db) {
    LOG_WARN("invalid argument. db is null");
    return RC::INVALID_ARGUMENT;
  ...
  for (size_t i = 0; i < select_sql.relations.size(); i++) {
    ...
    if (nullptr == table) {
      ...
      if (nullptr == table){ 
        ...
        // return RC::SCHEMA_TABLE_NOT_EXIST;
        if(db->find_table_by_alias(alias.c_str()) != nullptr){
          if (db->find_table_by_alias(alias.c_str())->name() != table_name1 && has_subselect == 0){
          LOG_WARN("alias already exists");
          return RC::INTERNAL;
          } else {
            //do nothing
